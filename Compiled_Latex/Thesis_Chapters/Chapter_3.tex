...
After recombustion at $525$\textdegree\ C for 1 hour to remove trace contaminant gases, isotope composition of \ce{CO2} contained in each RPO fraction was analyzed following standard procedures \citep{McNichol:1994ty,Pearson:1998vy}, where \ce{^{13}C} content is expressed in $\delta^{13}C$ per mille (\textperthousand) notation relative to Vienna Pee Dee Belemnite (VPDB) and \ce{^{14}C} content is expressed in fraction modern (Fm) notation following \citet{Stuiver:1977uh}. RPO fraction masses, \ce{\delta^{13}C} values, and Fm values were corrected for blank carbon contribution, and \ce{\delta^{13}C} was additionally corrected to ensure \ce{^{13}C} mass balance as incomplete oxidation to \ce{CO2} has been shown to exhibit a small fractionation effect \citep{Hemingway:2016rc}. Analytical uncertainty was propagated throughout all corrections.
...
Although \ce{^{13-12}\Delta E} is likely not identical for all compounds due to differences in the entropy and enthalpy of isotope substitution \citep{Tang:2000ua}, the estimated range of values for RPO analysis is small \citep[$0.3 - 1.8 \times 10^{-3}$ kJ mol\textsuperscript{-1};][]{Hemingway:2016rc}. We therefore assume $\ce{^{13-12}\Delta E} = 1.8 \times 10^{-3}$ kJ mol\textsuperscript{-1} for all RPO fractions, noting that a choice of $0.3 \times 10^{-3}$ kJ mol\textsuperscript{-1} would result in \ce{\delta^{13}C} values that are identical to those calculated here within analytical uncertainty.

$\ce{^{13/12}r}(t)$ can be determined using the ratio of carbon-normalized, isotope-specific decay rates as calculated in Equation \ref{Ch3Eq:12} by substituting $p_{0}(\ce{^{12}E})$ and $p_{0}(\ce{^{13}E})$ for $p_{0}(E)$. Because carbon is present as $\approx 99$\% \ce{^{12}C}, we set $p_{0}(\ce{^{12}E}) = p_{0}(E)$ such that $\frac{d \ce{^{12}G}(t)}{dt} = \frac{dG(t)}{dt}$. Corresponding $\frac{d \ce{^{13}G}(t)}{dt}$ can then be determined using $p_{0}(\ce{^{13}E}) = p_{0}(E + \ce{^{13-12}\Delta E})$. That is, \ce{^{13}C}-containing molecules decay at rates governed by a pdf of $E$ that is identical to $p_{0}(E)$ but has been shifted to the right by $1.8 \times 10^{-3}$ kJ mol\textsuperscript{-1}. We then correct the measured \ce{\delta^{13}C} values of each RPO fraction $f$ for kinetic isotope fractionation by dividing by the average $\ce{^{13/12}r}(t)$ value over the time of collection [written as $\mean{\ce{^{13/12}r}(t)_{f}}$]:
...
%	b.) However, a linear trend in Fm and d13C with increasing E indicates that each "peak" in this distribution is not isotopically unique. Rather, this suggests a mixture of 2 end members with unique d13C and Fm but overlapping E.


%		i.) Each end member here therefore has a complex E distribution but relatively narrow d13C and Fm. This is consistent with Pololu results and the fact that high-E material is petrogenic.

%	d.) Fatty acids are consistent with mostly biospheric at low E, requires increasing admixture of 13C-enrich, 14C- and fatty-acid-free material.

%3.) Lastly, JGOFS d13C still shows a separation of sources, despite non-first-order kinetics.

%4.) Beauty of this technique is the ability to parse apart sources that are isotopically and physically unique, even if they show some chemical overlap!
...
%Lines of argument / text outline:

%1.) First talk about soil sample, as it is purely biospheric OC and Fm reflects reservoir age:
%	a.) p0(E) is concentrated at low E, with a large, broad peak. This indicates that individual compound classes are not separable despite unique decomposition temperatures when analyzed as pure compounds (Williams, 2014). p0(E) peaks therefore represent some mixture of compounds.
%		i.) Components do not correspond to a simple set of chemical structures, rather a mix of sources!
%	b.) Fm is relatively flat, if anything increases with E, suggesting that thermal stability does not drive reservoir age in this sample. This further supports the idea that peaks represent a mixture, since we know individual compounds have variable 14C ages (Trumbore, Schmidt 2011)
%	c.) Still, see a small change in d13C with increasing E, suggesting that biospheric components with unique 13C composition could be relatively concentrated at higher or lower E despite a lack of distinct peaks.

%2.) Now introduce PB-60, as this is a more complicated mixture:
%	a.) In contrast to Pololu, PB-60 p0(E) is very broad and complex, reflecting an integration over more OC sources with different chemical structure.





%Because this sample is overlying igneous bedrock, any changes in Fm cannot reflect incorporation of OC\textsubscript{petro}. Rather, such trends are potentially due to differences in adsorption to mineral surfaces, preferential incorporation of bomb-derived \ce{^{14}C}, downward percolation of recently fixed dissolved OC, \textit{etc}. Although small in magnitude, the simultaneous decrease in \ce{\delta^{13}C} and in increase in Fm suggests that high-$E$ material might be less diagenetically altered, consistent with the observation that thermally labile compounds are generally more bioavailable \citep{Leifeld:2014hq}. We therefore highlight the potential for serial oxidation in general, and RPO in particular, to understand the processes controlling OC radiocarbon signatures and environmental turnover times \citep[\textit{e.g.}][]{Schmidt:2011gg}.
...
%By analyzing single compounds and standard reference materials, \citet{Williams:2014bq} observed that vascular-plant-derived cellulose and bulk leaf material pyrolyze between $\approx 300 - 400$\textdegree\ C ($\beta = 5$\textdegree\ C min\textsuperscript{-1}) and are thus described by low $E$ values. Although not directly comparable to our results due to potential differences between pyrolysis and combustion, this suggests that the low-$E$ end-member observed here mainly consists of C\textsubscript{3}-derived vascular plant material. 

%This is supported by the fact that tropical mixed and seasonal forest leaf biomass \ce{\delta^{13}C} values range from $\approx -29$ to $-32$\textperthousand\ \citep{Diefendorf:2010kb}, similar to those measured in RPO fractions 1--2. The average \ce{\delta^{13}C} value of long-chain \textit{n}-alkanoic acids ($\Sigma$LC\textsubscript{24--32}) extracted from this sample further agrees with this interpretation \citep[Figure \ref{Ch3Fig:9}A;][]{Galy:2011ix}, as plant-wax lipids display $\approx 3-4$\textperthousand\ lower \ce{\delta^{13}C} values than relative to bulk leaf material \citep{Diefendorf:2011hg}. The measured $\Sigma$LC\textsubscript{24--32} \textit{n}-alkanoic acid \ce{\delta^{13}C} value of $-31.7$\textperthousand\ therefore corresponds to a bulk leaf biomass value of $\approx -28$ to $-29$\textperthousand, again consistent with low-$E$ RPO fractions. We therefore hypothesize that labile C\textsubscript{3}-derived vascular plant material is the likeliest source of \ce{^{13}C}-depleted, low-$E$ OC in Narayani PB-60.

%In contrast, JGOFS MC-1 RPO \ce{\delta^{13}C} values are driven by a sharp increase in \ce{CaCO3} contribution at higher $E$ values, with fraction 5 perfectly matching the independently measured calcite value \citep[Figure \ref{Ch3Fig:9}B;][]{Sayles:2001ua}. Although carbon in this sample is present as $\approx 95$\% calcite, the \ce{\delta^{13}C} value of fraction 1 is near that expected for phytoplankton biomass OC in this region \citep{Rau:1989wr}, suggesting that RPO analysis can sufficiently separate low-$E$ OC from \ce{CaCO3} despite the potential for catalysis, matrix effects, and non-first-order kinetics. Still, we emphasize that non-first-order kinetics and mass-dependent \ce{CaCO3} elution temperatures do hinder our ability to interpret changes in OC isotope composition for carbonate-containing samples. For example, the \ce{\delta^{13}C} trend observed in Narayani PB-60 OC would be overprinted by an unknown contribution of \ce{CaCO3} to each fraction. We therefore do not recommend quantitatively interpreting RPO isotope and $p_{0}(E)$ results from carbonate-rich samples without independent constraints on end-member composition. 

%Lastly, Pololu 4169 displays a decreasing \ce{\delta^{13}C} trend with increasing $E$, opposite of the other samples analyzed (Figure \ref{Ch3Fig:9}C). Still, similar to Narayani PB-60, \ce{\delta^{13}C} does display a strong linear relationship with $E$ ($-0.04$\textperthousand\ [kJ mol\textsuperscript{-1}]\textsuperscript{-1}, $R^{2} = 0.845$, $p$-value = $2.7 \times 10^{-2}$). Although this \ce{\delta^{13}C} range is small, it likely reflects a combination of OC source and diagenetic effects. For example, because this site has undergone a transition from C\textsubscript{3} rainforest to C\textsubscript{4} pasture, it has been hypothesized that \ce{^{13}C}-enriched material has been transported downward in the soil column as dissolved OC and subsequently adsorbed to particle surfaces \citep{Chadwick:2007hc}. Depending on the chemical bonding environment of adsorbed OC, this could potentially explain the higher \ce{\delta^{13}C} values at low $E$ observed here. Alternatively, because diagenesis results in residual OC that is relatively enriched in \ce{^{13}C} relative to the initial composition, \ce{^{13}C} enrichment at low $E$ values could be the result of increased diagenetic alteration of this material. Consistent with the findings of \citet{Leifeld:2014hq}, this interpretation would suggest that thermally labile compounds are more biologically available. Overall, the $E$ vs. \ce{\delta^{13}C} trends observed here exemplify the potential for serial oxidation techniques to quantify the relationship between environmental processes and OC isotope signatures.




%While it remains possible that a small amount of OC\textsubscript{petro} is contained in earlier fractions with overlapping $p_{0}(E)$ distributions (\textit{i.e.} fractions 6--7, Figure \ref{Ch3Fig:8}A), this cannot explain the linear decrease in Fm with increasing $E$ throughout the entire analysis. Rather, this suggests that OC\textsubscript{bio} itself is composed of an increasing admixture of \ce{^{14}C}-free material with increasing $E$. This is supported by the large discrepancy between measured $\Sigma$LC\textsubscript{24--32} \textit{n}-alkanoic acid and estimated OC\textsubscript{bio} Fm values, which lead \citet{Galy:2011ix} to conclude that this sample must contain a significant amount of pre-aged OC\textsubscript{bio} that is free of $\Sigma$LC\textsubscript{24--32} \textit{n}-alkanoic acids, potentially derived from erosion of Tibetan Plateau soils. Additionally, a $\Sigma$LC\textsubscript{24--32} \textit{n}-alkanoic acid Fm value of $0.84 \pm 0.02$, similar to that of fractions 1--2, further indicates that these biomarkers predominantly track low-$E$ OC\textsubscript{bio}, while they should be nearly absent from material derived from the Tibetan Plateau due to the sparse vegetation in this region. The fact that OC\textsubscript{bio} \ce{\delta^{13}C} vs. $E$ and Fm vs. $E$ trends both linearly approach the OC\textsubscript{petro} end member strongly suggests that carbon originating as OC\textsubscript{petro} can become incorporated into OC\textsubscript{bio}. This interpretation is consistent with the experimental results of \citet{Petsch:2001eq}, who observed \ce{^{14}C}-free bacterial lipids during laboratory incubations of black shales and concluded that soil microbial communities can utilize OC\textsubscript{petro} as a carbon source. This process could explain the results observed here, as biological incorporation of OC\textsubscript{petro} into microbial biomass (here termed "fossil OC\textsubscript{bio}") would alter the chemical structure of this material, making it more oxidized and thus associated with lower $E$ values during RPO analysis.


%Similar to \ce{\delta^{13}C}, the Fm vs. $E$ relationship for Narayani PB-60 fractions 1--8 is strikingly linear, with a , followed by a slight reversal for fraction 9 (Figure \ref{Ch3Fig:9}D). Previously, both \citet{Galy:2008ff} and \citet{Rosenheim:2012kh} have independently estimated that $\approx 20 \pm 5$\% of OC contained in this sample is derived from eroded OC-rich bedrock ("OC\textsubscript{petro}") based on the fact that this material is \ce{^{14}C}-free by definition (\textit{i.e.} Fm\textsubscript{petro} $\equiv 0.0$). Because it has been shown that similarly condensed, aromatic "black" carbon pyrolyses above $\approx 650$\textdegree\ C \citep{Williams:2014bq}, here we attempt to verify the relative amount of OC\textsubscript{petro} by assuming that this end-member is contained exclusively in fractions 8--9 (\textit{i.e.} those eluting above $650$\textdegree\ C). We first correct for a minor input of charred biospheric OC ("OC\textsubscript{bio}") by assuming that this is equal to the mass-weighted mean Fm value for RPO fractions 1--7. Calculated mean Fm\textsubscript{bio} is equal to 0.453, statistically identical to that estimated previously by \citet{Galy:2008ff} ($0.492 \pm 0.056$; Figure \ref{Ch3Fig:9}D). We therefore correct for charring in fractions 8--9 by un-mixing these two end members using the equation m\textsubscript{petro} = m\textsubscript{\textit{f}}[1 -- (Fm\textsubscript{\textit{f}})/(Fm\textsubscript{bio})], where m\textsubscript{\textit{f}} and Fm\textsubscript{\textit{f}} are the measured mass and Fm value of each fraction (Table \ref{Ch3Tab:1}). This results in $23.7$\% of total OC present as OC\textsubscript{petro} ($25.1$\% if no charring correction is applied), identical to that calculated previously and supporting the idea that this material decomposes exclusively above $\approx 650$\textdegree\ C when analyzed using $\beta = 5$\textdegree\ C min\textsuperscript{-1}.
